...
public class AlertAnalyzer {
    ...
    public String analyze(String service) {
        ...
        String context = """
                ...
                [DB 커넥션 풀 사용률] %s
                """.formatted(
                safeQuery("up{job=\"%s\"}".formatted(service)),
                safeQuery("histogram_quantile(0.99, sum(rate(http_server_requests_seconds_bucket{job=\"%s\"}[5m])) by (le))".formatted(service)),
                safeQuery("sum(rate(http_server_requests_seconds_count{job=\"%s\",status=~\"5..\"}[5m]))".formatted(service)),
                safeQuery("avg_over_time(process_cpu_usage{job=\"%s\"}[5m])".formatted(service)),
                safeQuery("hikaricp_connections_active{job=\"%s\"} / hikaricp_connections_max{job=\"%s\"}".formatted(service, service))
        );
        ...
        return chatClient.prompt()
                .system("""
                        ...
                        "쿼리 오류"가 있으면 Prometheus 자체의 장애 가능성으로 보고하라.
                        """)
    ...
    /**
     * 진단 쿼리 하나가 실패해도 나머지로 분석과 Slack 통지가 계속되도록 예외를 흡수한다
     */
    private String safeQuery(String promql) {
        try {
