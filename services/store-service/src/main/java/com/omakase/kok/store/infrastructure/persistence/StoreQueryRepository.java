...
public class StoreQueryRepository {
    ...
    public Page<Store> search(StoreSearchCondition condition, Pageable pageable) {
        ...
        // amenity JOIN 시 1:N 중복 로우 방지 → selectDistinct, JOIN 없을 땐 select (DISTINCT + ORDER BY 비SELECT 식 금지)
        JPAQuery<Store> contentQuery = hasAmenityFilter
        ...
        JPAQuery<Long> countQuery = hasAmenityFilter
                ? queryFactory.select(store.countDistinct()).from(store).leftJoin(store.amenities, amenity)
                : queryFactory.select(store.count()).from(store);
